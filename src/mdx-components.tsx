...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    ...
    img: async ({ className, src, alt, ...props }) => {
      const img = await import(`@/projects/img/${src}`);
      return (
        <Image
    ...
    code: ({ className, ...props }) => (
      <code
        className={clsx(
          "relative rounded-sm border bg-zinc-300 bg-opacity-25 py-[0.2rem] px-[0.3rem] font-mono text-sm text-zinc-600",
          className,
        )}
        {...props}
      />
